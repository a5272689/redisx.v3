...
func (p *Pool) Info(sections ...string) (map[string]string, error) {
	newargs:=[]interface{}{}
	for _,tmpsection:=range sections{
		newargs=append(newargs,tmpsection)
	}
	rs:=p.Cmd("info",newargs...)
...
func (p *Pool) ConfigGet(parameter string) (map[string]string, error) {
	rs:=p.Cmd("config","get",parameter)
	result,err:=rs.List()
